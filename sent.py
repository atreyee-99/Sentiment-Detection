...
if __name__ == "__main__" : 
	...
	gui.config(background = "#107d2e") 

	# set the name of tkinter GUI window 
	gui.title("Sentiment Detector GUI Application") 

	# Set the configuration of GUI window 
	gui.geometry("250x400") 

	# create a label : Enter Your Task 
	enterText = Label(gui, text = "Enter the Sentence", 
									bg = "light blue") 
	...
	check = Button(gui, text = "Check Sentiment", fg = "Black", 
						bg = "White", command = detect_sentiment) 
	check.bind('<Enter>', onButton)
	check.bind('<Leave>', leaveButton)
	...
	clear = Button(gui, text = "Clear", fg = "Black", 
					bg = "White", command = clearAll) 
	clear.bind('<Enter>', onButtoncl)
	clear.bind('<Leave>', leaveButtoncl)
	...
	Exit.bind('<Enter>', onButtonex)
	Exit.bind('<Leave>', leaveButtonex)
